...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        # Configure camera with 180 degree rotation (vflip and hflip) for display rotation
        config = self.picam2.create_preview_configuration(
            main={"size": (640, 480)},
            transform=Transform(vflip=True, hflip=True)  # 180 degree rotation
        )
        self.picam2.configure(config)
        self.picam2.start()
        print("[CAMERA] PiCamera2 initialized and started with 180 degree rotation")
        ...
        # Create a rotated status label (placed at bottom instead of top)
        self.status_label = tk.Label(self.canvas, text="", font=("Helvetica", 24), bg="black", fg="white")
        self.status_label.place(x=20, y=screen_height-70)  # Moved from top to bottom
        
        # Add exit button (moved to bottom-left instead of bottom-right)
        ...
        self.exit_button.place(x=20, y=20)  # Moved to top-left
    ...
    def update_frame(self):
        if recognition_active:
            ...
            # Frame is already rotated by the camera configuration
